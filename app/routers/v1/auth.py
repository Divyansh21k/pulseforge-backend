...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    user = Participant(full_name=payload.full_name, email=payload.email, hashed_password=hash_password(payload.password), organization=payload.organization, role=payload.role)
    db.add(user)
    db.commit()
    db.refresh(user)
...
def me(current_user: Participant = Depends(get_current_user)):
    return {"id": current_user.id, "full_name": current_user.full_name, "email": current_user.email, "role": current_user.role, "organization": current_user.organization}
